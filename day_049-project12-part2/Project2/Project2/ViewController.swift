...
class ViewController: UIViewController {
    ...
    private func updateHighscoreLabel(with score: Int) {
        highscoreLabel.text = "High Score: \(score)"
    }
    
    private func getSavedHighScore() {
        let defaults = UserDefaults.standard
        highscore = defaults.integer(forKey: "highscore")
        updateHighscoreLabel(with: highscore)
    }
    
    private func saveNewHighScore(_ newHighScore: Int) {
        let defaults = UserDefaults.standard
        defaults.set(newHighScore, forKey: "highscore")
        updateHighscoreLabel(with: newHighScore)
        
        // Show alert beat previous highscore
        let ac = UIAlertController(title: "New High Score!", message: nil, preferredStyle: .alert)
