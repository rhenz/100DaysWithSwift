...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        LocalNotifManager.shared.checkNotificationsAuthorizationStatus { status in
            if status == .notDetermined {
                LocalNotifManager.shared.registerLocalNotif()
            }
        }
    }
    ...
    private func presentGameCompleteAlert() {
        ...
        // Challenge #3
        // Set Local Notif to Play Again
        LocalNotifManager.shared.checkNotificationsAuthorizationStatus { status in
            switch status {
            case .authorized:
                print("Scheduling to play again..")
                LocalNotifManager.shared.scheduleLocalNotif()
            default:
                print("Must register for local notification first")
            }
        }
    }
