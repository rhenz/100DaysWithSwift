...
class GameScene: SKScene {
    ...
    // Challenge #1
    enum EnemyPos {
        static let startX = 64
        static let endX = 960
        static let yPosition = -128
        
        static let endPosition1: CGFloat = 256
        static let endPosition2: CGFloat = 512
        static let endPosition3: CGFloat = 768
    }
    ...
    private func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        let randomPosition = CGPoint(x: Int.random(in: EnemyPos.startX ... EnemyPos.endX), y: EnemyPos.yPosition)
        ...
        if randomPosition.x < EnemyPos.endPosition1 {
            randomXVelocity = Int.random(in: 8...15)
        } else if randomPosition.x < EnemyPos.endPosition2 {
            randomXVelocity = Int.random(in: 3...5)
        } else if randomPosition.x < EnemyPos.endPosition3 {
            randomXVelocity = -Int.random(in: 3...5)
        } else {
            randomXVelocity = -Int.random(in: 8...15)
        }
        ...
        let yVelocityMultiplier = enemyType == 6 ? fastEnemyVelocityMultiplier : randomYVelocity // Challenge #2
        enemy.physicsBody = SKPhysicsBody(circleOfRadius: 64)
        enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * 40, dy: yVelocityMultiplier * 40)
    ...
    private func showGameOver() {
        let gameOverLabelNode = SKLabelNode(fontNamed: Styles.FontName.chalkduster)
        gameOverLabelNode.text = "Game Over"
        gameOverLabelNode.horizontalAlignmentMode = .left
        gameOverLabelNode.fontSize = K.gameScoreFontSize
        addChild(gameOverLabelNode)
        gameOverLabelNode.position = K.gameOverPosition
    }
}
...
fileprivate enum GameSceneConstants {
    ...
    static let gameScorePosition: CGPoint = CGPoint(x: 80, y: 8)
    static let gameOverPosition: CGPoint = CGPoint(x: 400, y: 372)
    ...
    static let yGravity: CGFloat = -6.0 // Earth's gravity is -9.98
